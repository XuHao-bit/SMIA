...
def train(args, net, optimizer, trainloader, epoch, device):
    net.train()
    # print(net.RRT)
    ...
    for i in decay:
        for j in kl_reg:
            ...
            if args.is_shadow:
                ...
                # net = DiffNet(config=config, args=args, device=device)
            net = net.to(device)    
            # Learning Algorithm
            ...
            for epoch in range(args.num_epoch):
                train_loss = train(args, net, optimizer, train_loader, epoch, device)
